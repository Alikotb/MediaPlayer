...
import com.audio.tunoo.mediaplayer.databinding.FragmentAudioPlayerBinding
import com.audio.tunoo.mediaplayer.features.Response
import com.audio.tunoo.mediaplayer.features.audio_player.view.PlayListAdapter
import com.audio.tunoo.mediaplayer.features.common.createPlaylistDialog
import com.audio.tunoo.mediaplayer.features.common.showCustomSnackbar
...
import com.audio.tunoo.mediaplayer.service.MyMediaService
import com.audio.tunoo.mediaplayer.utils.getAlbumArt
import com.audio.tunoo.mediaplayer.utils.getCustomGradientList
import com.audio.tunoo.mediaplayer.utils.toMinutesAndSeconds
...
class AudioPlayerFragment : Fragment() {
    ...
    private fun updateUiForTrack(track: AudioDto) {
        ...
        binding?.trackArtistAudioPlayerTv?.text = track.artist ?: getString(R.string.unknown_artist)
    ...
    private fun showAddToPlaylistDialog(context: Context, audio: AudioDto) {
        ...
        lifecycleScope.launch {
            playlistViewModel.namesState.collectLatest { response ->
                if (response is Response.Success) adapter.updateList(response.data)
            }
        }
        ...
        view.findViewById<LinearLayout>(R.id.new_playlist_row).setOnClickListener {
            createPlaylistDialog(context) { name ->
                if (name.isNotBlank()) playlistViewModel.addNewList(
                    PlaylistDto(
                        name,
                        listOf(audio)
                    )
    ...
    private fun setAsRingtone(audio: AudioDto) {
        ...
        if (newUri != null) {
            ...
            Toast.makeText(requireContext(), getString(R.string.ringtone_set), Toast.LENGTH_SHORT).show()
        } else {
            Toast.makeText(requireContext(), getString(R.string.failed_to_set_ringtone), Toast.LENGTH_SHORT).show()
        }
    }
