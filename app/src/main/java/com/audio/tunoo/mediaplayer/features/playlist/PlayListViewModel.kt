package com.audio.tunoo.mediaplayer.features.playlist
...
class PlayListViewModel(private val repo: IRepository) : ViewModel() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            if (playlistDto.playlistName.isNotBlank()&&playlistDto.playlistName.isNotEmpty()) {
                if (isPlaylistExists(playlistDto.playlistName)) {
                    _snackbarFlow.emit(R.string.already_exists)

                } else {
                    repo.insertNewPlayList(playlistDto)
                    _snackbarFlow.emit(R.string.added_successfully)
                    getAllPlayList()
                }
            } else {
                _snackbarFlow.emit(R.string.enter_playlist_name)
                return@launch
            }
        }
    }
    ...
    fun deletePlaylist(playlistDto: PlaylistDto) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            getAllPlayList()
            _snackbarFlow.emit(R.string.deleted_successfully)
